...
.directive("wbMultiselectPicklist",function(appConfig, $timeout){
  ...
  return {
    ...
    controller : function($scope) {
        ...
        $scope.onChangeText = function(input){
            for(var i = 0; i < $scope.options.length; ++i){
                var optVal = $scope.options[i].label.toLowerCase();
                if(input){
                    if(optVal.indexOf(input.toLowerCase()) != -1){
                        $scope.options[i].hideOption = false;
                    }else{
                        $scope.options[i].hideOption = true;
                    }
                }else{
                    $scope.options[i].hideOption = false;
                }
    ...
    templateUrl : 'wb-multiselect-picklist.html',
    replace:true,
