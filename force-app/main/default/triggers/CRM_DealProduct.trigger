    ...
    if((Trigger.isBefore && Trigger.isInsert)||(Trigger.isBefore && Trigger.isUpdate)) { 
        ...
        if (allRecordSize == physicalPOSDFIRecordSize) {
            if(!ApexUtil.isTriggerInvoked && !(System.isBatch() && (CRM_ApplicationUtility.isDecayCurveReforcastingBatch || CRM_ApplicationUtility.isJVAutoPhysicalNRBatch))){
                ...
                // W-017775-Hybrid label changes for manual Add Products via Template or Upload Extract for DFI/POS/Planning Deal for CRM JV
                CRM_UploadExtractPOSandDFI.addValidationErrorForDRHValue(trigger.new);
            }
            return;
        }
    }
    ...
    if(Trigger.isBefore && Trigger.isDelete) { 
        ...
        if (allRecordSize == physicalPOSDFIRecordSize) {
            if(!ApexUtil.isTriggerInvoked && !(System.isBatch() && (CRM_ApplicationUtility.isDecayCurveReforcastingBatch || CRM_ApplicationUtility.isJVAutoPhysicalNRBatch))){
                ...
                CRM_UploadExtractPOSandDFI.validateDealProductDeletionFromRevisedDeal(trigger.old);
